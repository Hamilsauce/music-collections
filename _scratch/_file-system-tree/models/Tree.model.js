...
export class Tree {
  ...
  export append(target, node) { target.appendChild(node) }

  export listen(node, event, handler) {
    node.addEventListener(event, handler);
    return () => node.removeEventListener(event, handler)
  }

  export detach(node) { node.parentNode.removeChild(node) }

  export getDataType(val) {
