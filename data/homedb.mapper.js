  ...
  return folder$
    .pipe(
      ...
      map(({ folder, childrenRefs }) => ({
        parent: { ...folder, fileRefs: childrenRefs.files || [] },
...
// getRecursive(rootFolder$)//.subscribe(x =>
//   console.log('END OF RECURSIVE TREE', x)
// )
...
// const parentFolders$ = buildTreePipe(foldersByParentId$)
// .subscribe(x => console.log('END OF FOLDER REF PIPE', x))




// 
// mapCollections$.subscribe(x => console.log('END OF mapCollections$ REF PIPE', x))
    ...
    mergeMap((group$) => group$
      .pipe(
        ...
        map(arr => {
          ...
          // const ids = arr.map(_ => _.parentId)
          // console.log('ids', ids)
...
// mapCollections$.disconnect = () => homeDbSource.disconnect();

const parentFolders$ = addFileDataPipe(buildTreePipe(foldersByParentId$))

const folderTreeSubject$ = new Subject()

const folderTreeSubscription = getRecursive(rootFolder$).subscribe(folderTreeSubject$)
